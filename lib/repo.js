...
module.exports = {
  ...
  createGitignore: async () => {
    ...
    if (GitIgnorAnswers.ignore.length)
      gitignoreFileContent += '# Others\n' + GitIgnorAnswers.ignore.join('\n');

    if (gitignoreFileContent) fs.writeFileSync('.gitignore', gitignoreFileContent);
...
const createRepoForAuthenticatedUser = async (data) => {
  ...
  try {
    ...
    if (errs) {
      if (Array.isArray(errs)) errs.forEach((err) => dbg.err(err));
      else dbg.err(err);
    } else {
      dbg.err(err);
    }
